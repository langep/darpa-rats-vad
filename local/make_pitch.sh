...
if [ $# -lt 1 ] || [ $# -gt 3 ]; then
   ...
   echo "Note: <log-dir> defaults to <data-dir>/log, and <pitchdir> defaults to <data-dir>/data"
   echo "Options: "
...
if [ -f $data/segments ]; then
  ...
  $cmd JOB=1:$nj $logdir/make_pitch_${name}.JOB.log \
    ...
    compute-pitch-feats $vtln_opts --verbose=2 --config=$pitch_config ark:- ark:- \| \
  ...
  $cmd JOB=1:$nj $logdir/make_pitch_${name}.JOB.log \
    compute-and-process-kaldi-pitch-feats  $vtln_opts --verbose=2 --config=$pitch_config \
     scp,p:$logdir/wav_${name}.JOB.scp ark:- \| \
      copy-feats $write_num_frames_opt --compress=$compress ark:- \
      t,ark,scp:$pitchdir/raw_pitch_$name.JOB.ark,$pitchdir/raw_pitch_$name.JOB.scp \
      || exit 1;
fi
